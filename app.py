...
def match_generic_skills(resume_text, generic_skills):
    resume_text_lower = resume_text.lower()
    matched = []
    for skill in generic_skills:
        if skill.lower() in resume_text_lower:
...
def calculate_ats(resume_text, job_description):
    ...
    matched_generic = match_generic_skills(resume_text, GENERIC_SKILLS)
    
    keyword_score = len(matched_keywords)/max(len(job_keywords),1)*50
    generic_score = len(matched_generic)/max(len(GENERIC_SKILLS),1)*30
    ...
    if len(matched_generic)<5:
        tips.append("Add more soft skills like communication, teamwork, leadership.")
    if missing_keywords:
        tips.append("Include missing job-specific keywords naturally in experience or skills.")
...
iface = gr.Interface(
    ...
    description="Upload your resume and paste the job description to calculate ATS score, see missing keywords, and improvement tips."
)
